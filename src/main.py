...
def main():
    ...
    create_db('vacancieshh', params)

    save_data_to_db(companies_data, vacancies_data, 'vacancieshh', params)###
    conn = psycopg2.connect(dbname='vacancieshh', **params)
    db_m = DBManager(conn)
    ...
    while True:
        ...
        if user_input == '1':
            ...
            for vacancy in higher_salary_vacancies:
                print(f"Вакансия: {vacancy[2]}, Зарплата: {vacancy[4]}-{vacancy[5]},"
                      f"Ссылка на вакансию: {vacancy[7]}")
        elif user_input == '5':
